...
邮件订阅当前推荐使用无需域名的Buttondown，腾讯云SES适配仍保留为以后有域名时的备用。邮件功能默认关闭，不会影响网页、RSS和每日文献任务。首次启用请阅读 [Buttondown邮件订阅配置教程](./docs/Buttondown邮件订阅配置教程.md)。
...
1. 优先检索最近 30 天
2. 不足 5 篇或无法组成“4 篇临床证据＋1 篇基础研究”时先扩展到最近 180 天，仍不足再扩展到最近 360 天
3. 按 PMID、DOI 和标题相似度去重
4. 永久排除已经正式发布或发送过的文章；候选、Demo、失败和未发布记录不占用历史
5. 临床原始研究、系统评价、Meta分析、指南和共识共同计入临床证据；分别选择影响因子最高的4篇临床证据和1篇基础研究
6. 要求期刊影响因子严格大于 5；数值未知、期刊未配置或指标年份过旧时排除。影响因子相同时依次比较综合质量、证据质量和发表日期
7. 验证 DOI、识别 PMC 全文
8. 通过统一 `generateAI()` 接口完成结构化提取、完整摘要翻译、精读V2生成和统一事实复核
9. 更新网页数据、往期简报和RSS
10. 完整邮件展示最终整合内容，不展示新旧两套重复栏目，也不提供项目网页入口
11. 360 天仍不足时少发但继续推送，不降标准、不跨类别补位
12. 保存本次运行记录、Token汇总和正式推送历史
...
- `config/journal-metrics.json`：期刊影响因子硬筛选门槛、指标年份和核验来源；默认仅推荐影响因子严格大于5且数值可核验的期刊
...
- `src/literature/statistics.mjs`：经过代码审核的统计方法通俗解释词典
- `.env.local`：运行模式及服务密钥
- `docs/项目接续包/`：供新对话、上下文压缩、跨AI或隔月继续开发使用的完整交接资料
...
当前不包含用户注册、登录、微信、小程序、支付、收藏、社区或个性化推荐。邮件订阅采用Buttondown二次确认、托管退订和API发送；API Key不进入公开网页或日志。腾讯云SES与加密名单方案仍作为备用，RSS可独立使用。

## 费用结论

不需要购买服务器，当前也没有开通EdgeOne付费增值项。实测腾讯云中国站的默认 `*.edgeone.cool` 地址仅提供3小时限时预览；在“不购买域名”的限制下，长期公开网址目前尚未完成，不能把预览地址冒充正式地址。AI API仍是当前唯一已经预期发生的费用；如管理员以后提供已持有且符合接入要求的域名，才可继续完成中国站长期公开访问。免费政策可能调整，管理员仍需按教程每月检查腾讯云与GitHub控制台。
