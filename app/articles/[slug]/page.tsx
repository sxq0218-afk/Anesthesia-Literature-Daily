...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <header className="article-header"><div className="shell article-header-inner">
      ...
      <div className="meta-line"><span className="category-pill">{article.category}</span><span>{article.journal}</span><span>{article.publishedDate || article.year}</span><span>精读约 {article.readingTime}</span></div>
      <h1>{article.title}</h1>
    ...
    <div className="shell article-layout">
      <article className="article-body">
        ...
        {isReal && <section id="source"><h2>原文入口与全文状态</h2><p>PMID：{article.pmid || "无"}<br />DOI：{article.doi || "PubMed未提供"}</p>{article.urls?.openFullText ? <p>已在 PubMed Central 发现合法开放全文，可通过上方“免费全文”按钮访问。</p> : <div className="disclaimer">当前未发现开放全文，仅提供PubMed摘要和DOI入口。</div>}</section>}
