...
4. 定时为UTC 20:37，即次日北京时间约04:37。
5. GitHub定时任务可能延迟，不能承诺精确到分钟。
6. 正式生成、AI连接和Buttondown连接工作流只允许在`main`运行。

成功摘要应显示：

- 内容状态`production`；
- 入选篇数；
- 检索范围30天，或依次扩展到180天、360天；
- 当前模型；
- 输入、输出和总Token；
- Buttondown投递状态。
...
- 系统自动降级为开放全文重点章节节选，不会因此中断整期。
- 摘要完整保留，全文优先使用Methods、Results、Discussion和Limitations；无法识别章节时使用安全长度内的前后文节选。
